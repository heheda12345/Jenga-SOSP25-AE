...
class MllamaTextCrossAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            raise NotImplementedError("TODO: layer_id")
...
class MllamaForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    def get_full_text_row_masked_out_mask(
        ...
        decoder_attn_metadata: AttentionMetadata,
        device: torch.device,
    ) -> torch.Tensor:
        full_text_row_masked_out_mask = torch.ones(
            (decoder_attn_metadata.num_prefill_tokens, 1), dtype=torch.bool)
        start_pos = 0
        for seq_len, encoder_seq_len in zip(
                decoder_attn_metadata.seq_lens,
                encoder_attn_metadata.encoder_seq_lens):
        ...
        # For image-present prefill.
            ...
            full_text_row_masked_out_mask = \
                self.get_full_text_row_masked_out_mask(
                    encoder_attn_metadata, decoder_attn_metadata, input_ids.device)

            cross_attention_mask, kv_range_for_decode = \
                self.get_cross_attention_mask(
                    input_ids, decoder_attn_metadata, num_tiles,
...
def skip_attention_mask(sparse_mask: List[List[int]]) -> bool:
    ...
    for mask in sparse_mask:
        ...
        # we can't skip mask.
        if (mask[0][0] != 0 and mask[0][0] != 1) or mask[0][1] != -1:
            return False
    return True
