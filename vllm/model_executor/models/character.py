...
def custom_block_manager_for_character(model_config: ModelConfig,
    ...
    if hasattr(model_config.hf_config, 'sliding_window'):
        ...
        elif isinstance(sliding_window, (list, tuple)):
    ...
    num_self = 0
    num_sliding = 0
    for layer_ids in model_config.hf_config.kv_share:
        if sliding_window[layer_ids[0]] is None:
            num_self += 1
            manager = SelfAttentionManager(model_config, parallel_config,
                                           cache_config.cache_dtype,
                                           cache_config.block_size)
        else:
            num_sliding += 1
            manager = SlidingWindowManager(model_config, parallel_config,
                                           cache_config.cache_dtype,
                                           cache_config.block_size,
                                           sliding_window[layer_ids[0]])
        custom_managers[str(layer_ids[0])] = manager
    if scheduler_config.max_page_allocator:
        for i in range(num_self, num_sliding):
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if hasattr(config, "sliding_window"):
            ...
            if isinstance(config.sliding_window, (list, tuple)):
                sliding_window = config.sliding_window[layer_id % len(
...
class LlamaModel(nn.Module):
    ...
    def __init__(
        ...
        self.layer_id_sharing = {}
        for layer_ids in config.kv_share:
            for layer_id in layer_ids:
                self.layer_id_sharing[layer_id] = layer_ids[0]
    ...
    def forward(
        ...
        for i in range(self.start_layer, self.end_layer):
            layer = self.layers[i]
            shared_layer_id = self.layer_id_sharing[i]
            shared_layer_id = str(shared_layer_id)
            hidden_states, residual = layer(positions, hidden_states,
                                            kv_caches[shared_layer_id],
                                            attn_metadata[shared_layer_id],
                                            residual, shared_layer_id)
...
@BLOCK_MANAGER_REGISTRY.register_block_manager(
    custom_block_manager_for_character)
class CharacterForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
