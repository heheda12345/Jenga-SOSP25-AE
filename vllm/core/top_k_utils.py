...
class TopKCalculator:
    def __init__(self, num_layers: int = 80) -> None:
        ...
        self.past_step_layer_topk = {} # seq ID -> its topk in the past step for each layer (for v3 scheduler)
    ...
    def get_top_k_for_seq(
        ...
        if update:
            ...
            self.past_step_layer_topk[seq_id] = top_k_wo_none_list
    ...
    def get_past_step_all_top_k(self, seq_id: int) -> int:
        """
        Return the topk for this seq_id in the past step for this layer
        """
        return self.past_step_layer_topk[seq_id]
    
    def get_past_step_layer_top_k(self, seq_id: int, layer_id: int) -> int:
        """
        Return the topk for this seq_id in the past step for this layer
        """
        return self.past_step_layer_topk[seq_id][layer_id]
