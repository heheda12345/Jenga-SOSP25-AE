...
@pytest.mark.parametrize(
    "model",
    [
        "meta-llama/Llama-4-Scout-17B-16E-Instruct",  # local_chunk + full attention # noqa
    ])
@pytest.mark.parametrize("batch_size", [5])
@pytest.mark.parametrize("seed", [1])
@large_gpu_test(min_gb=48)
def test_sliding_window_retrival_large_models(monkeypatch, model, batch_size,
                                              seed):
    """
    The test does a bunch of assignments "x1 = 10\nx2 = 33\n..." and then
    asks for value of one of them (which is outside the sliding window).
...
def check_length(prompts: list[str], llm: LLM, sliding_window: int):
    ...
    prompt_length = [len(tokenizer.encode(prompt)) for prompt in prompts]
    print("prompt_length", prompt_length)
    assert any(l > sliding_window
               for l in prompt_length), "Prompt is too short for test"
    assert all(l <= max_model_len
               for l in prompt_length), "Prompt is too long for test"
